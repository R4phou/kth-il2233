...
void print_to_file_som(t_pos* assignment, int n_samples, string filename){
    ...
    file.open(filename);
    for(int i=0; i<n_samples; i++){
        file << assignment[i].x << endl; //"," << assignment[i].y << endl;
    }
    file.close();
}
...
int main(){
    ...
    int K = 3;
    int max_iter = 100;
    int assignment[150] = {0};

    // Print the assignment for kmeans

    clock_t start, end;
    start = clock();
    kmeans(assignment, 3, max_iter, 30, 128, BME);
    // kmeans(assignment, K, max_iter, 150, 4, iris);
    end = clock();
    double time_taken = double(end - start) / CLOCKS_PER_SEC * 1000; // convert to milliseconds
    cout << "Time taken by kmeans: " << time_taken << "ms" << endl;
    print_to_file_kmeans(assignment, 30, "./output/kmeans_assignment_BME.txt");
    // print_to_file_kmeans(assignment, 150, "./output/kmeans_assignment_iris.txt");

    // Test the SOM function
    max_iter = 400;
    float lr = 0.1;
    float sigma = 1.0;
    t_pos assignment2[150];

    
    start = clock();
    SOM(assignment2, BME, 30, 128, 3, 1, max_iter, lr, sigma);
    // SOM(assignment2, iris, 150, 4, 3, 1, max_iter, lr, sigma);
    end = clock();
    time_taken = double(end - start) / CLOCKS_PER_SEC * 1000; // convert to milliseconds
    cout << "Time taken by SOM: " << time_taken << "ms" << endl;
    print_to_file_som(assignment2, 30, "./output/som_assignment_BME.txt");
    // print_to_file_som(assignment2, 150, "./output/som_assignment_iris.txt");
